...
if [ "$1" == "Start-Interactive-Script-Test" ]; then
  echo
  echo "------------------------------------[[[[ Start-Interactive-Script-Test ]]]]------------------------------------"
  echo
  echo "This command will run the Start-Interactive-Script-Test with a specific script file name and Hardhat Network name. This run started on $TIMESTAMP."
  echo
  source ./.env
  echo "" > ./logs/hardhat-interactive-script-test-"$3".log
  echo "Now running the $2 interactive script to interact with the Lottery smart contract..." >> ./logs/hardhat-interactive-script-test-"$3".log
  echo "" >> ./logs/hardhat-interactive-script-test-"$3".log
  yarn hardhat run scripts/$2 --network $3 >> ./logs/hardhat-interactive-script-test-"$3".log
  echo "" >> ./logs/hardhat-interactive-script-test-"$3".log
  sleep 2  
  cat ./logs/hardhat-interactive-script-test-"$3".log
  echo
  echo
  TIMESTAMP2=$(date)
  echo "This run ended on $TIMESTAMP2."
...
usage_explanation() {
  ...
  echo
  echo "You can view just this help menu again (without triggering any automation) by running 'bash ./Lesson8CodeSessions_start-deployment-scripts.sh -h' or 'bash ./Lesson8CodeSessions_start-deployment-scripts.sh -h --help'."
  echo
  echo "bash ./Lesson8CodeSessions_start-deployment-scripts.sh Stop-Local-Blockchain-Nodes-Clean-Environment"
  echo "bash ./Lesson8CodeSessions_start-deployment-scripts.sh Install-Tools-On-MacOS-Or-Linux"
  echo "bash ./Lesson8CodeSessions_start-deployment-scripts.sh Start-Deployment-On-A-Specific-Network hardhat"
  echo "bash ./Lesson8CodeSessions_start-deployment-scripts.sh Start-Deployment-On-Real-Ethereum-Testnet"
  echo "bash ./Lesson8CodeSessions_start-deployment-scripts.sh Start-Hardhat-Test"
  echo "bash ./Lesson8CodeSessions_start-deployment-scripts.sh Start-Interactive-Script-Test enter.js localhost"
  echo
  ...
  echo
  echo "bash ./Lesson8CodeSessions_start-deployment-scripts.sh Stop-Local-Blockchain-Nodes-Clean-Environment && bash ./Lesson8CodeSessions_start-deployment-scripts.sh Start-Deployment-On-A-Specific-Network hardhat"
  echo
  echo "You can also use the provided Lesson8CodeSessions_start-automation-build-pipeline.sh to run all the above commands in a sequence similar to a build pipeline."
  echo
  echo "bash ./Lesson8CodeSessions_start-automation-build-pipeline.sh"
  echo
  echo
}
